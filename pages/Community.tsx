...
const FestivalWidget = () => {
  ...
  const fetchFestivals = useCallback(async () => {
    ...
    try {
      ...
      const prompt = `Lập danh sách 10 lễ hội văn hóa lớn của Việt Nam trong năm ${today.getFullYear()}. Cấu trúc JSON chuẩn xác, bắt buộc dùng ngoặc kép (") cho key và value. KHÔNG BÌNH LUẬN, KHÔNG DÙNG MARKDOWN. Định dạng mẫu: [{"id": "le-hoi-1", "name": "Tên lễ hội", "solarDate": "YYYY-MM-DD", "lunarDateStr": "Ngày/Tháng Âm lịch", "location": "Tỉnh/Thành phố"}]`;
      ...
      const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${API_KEY}`, {
        ...
        body: JSON.stringify({ 
          ...
          generationConfig: { 
            temperature: 0.7,
            responseMimeType: "application/json" // VŨ KHÍ TỐI THƯỢNG: Ép AI trả JSON sạch
          } 
        })
      });
      
      if (!res.ok) throw new Error(`Lỗi Google API: ${res.status}`);
...
const QuizWidget = () => {
  ...
  const initQuiz = useCallback(async () => {
    ...
    try {
      ...
      const prompt = `Tạo 5 câu hỏi trắc nghiệm ĐỘC ĐÁO về văn hóa của 54 dân tộc Việt Nam. Cấu trúc JSON chuẩn xác, bắt buộc dùng ngoặc kép (") cho key và value. KHÔNG BÌNH LUẬN, KHÔNG DÙNG MARKDOWN. Định dạng mẫu: [{"id": 1, "question": "...", "options": ["A", "B", "C", "D"], "correctAnswerStr": "A", "explanation": "..."}]`;
          ...
          <div className="animate-slide-up flex-1">
            ...
            {selected && (
               <div className="mt-3 md:mt-4 animate-fade-in">
                  ...
                  <button onClick={() => { setQIndex(i => i + 1); setSelected(null); }} className="w-full bg-primary text-white py-2.5 md:py-3 rounded-xl font-black uppercase text-[10px] md:text-xs tracking-widest hover:brightness-110 shadow-lg active:scale-95 transition-transform">Tiếp tục</button>
               </div>
            )}
          </div>
        ) : null}
    ...
    <div className="bg-white border-2 border-gold/10 rounded-2xl md:rounded-[2rem] p-4 md:p-6 shadow-xl relative overflow-hidden group hover:border-gold/30 transition-colors w-full">
      <div className="flex flex-col md:flex-row gap-3 md:gap-4 relative z-10">
         ...
         <div className="flex-1 w-full">
           ...
           {img && <div className="relative mt-2 h-24 md:h-32 bg-black/5 rounded-xl border border-gold/20 overflow-hidden"><img src={img} className="h-full w-full object-contain"/><button onClick={()=>setImg('')} className="absolute top-2 right-2 text-white bg-black/50 p-1.5 rounded-full hover:bg-red-500 transition-colors"><span className="material-symbols-outlined text-xs md:text-sm">close</span></button></div>}
...
const PostCard = React.memo(({ post, currentUser, onDelete }: { post: Post, currentUser: any, onDelete: (id: string) => void }) => {
  ...
  return (
    <article className="break-inside-avoid mb-6 bg-white border border-gold/15 rounded-2xl md:rounded-[2rem] overflow-hidden shadow-md hover:shadow-xl transition-all group animate-slide-up w-full">
       <div className="p-4 md:p-5 flex items-center justify-between">
          ...
          <button onClick={() => onDelete(post.id)} className="text-red-400 hover:text-red-600 transition-colors"><span className="material-symbols-outlined">delete</span></button>
       </div>
...
const Community: React.FC = () => {
  ...
  const loadPosts = useCallback(async () => {
    setIsLoading(true);
    try {
      // ĐỒNG BỘ VỚI COMMUNITYSERVICE.TS CỦA BẠN (Dùng bảng bai_viet)
      ...
      console.warn("Lỗi load bài viết từ Database:", e); 
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const handlePost = async (content: string, img: string = '') => {
      try {
        ...
        loadPosts(); // Tải lại trang sau khi đăng
      } catch (error) {
  ...
  return (
    <div className="min-h-screen font-display bg-[#F7F3E9] py-8 md:py-12 px-4 md:px-6 relative">
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        {/* BỐ CỤC CHUẨN: BÀI ĐĂNG NẰM TRÊN CÙNG Ở MOBILE */}
        <div className="flex flex-col lg:flex-row gap-8 md:gap-10">
          ...
          <div className="flex-1 w-full flex flex-col gap-6 md:gap-8">
             ...
             {isLoading ? (
               <div className="text-center py-10 font-bold text-gold animate-pulse">Đang tải bài viết từ Database...</div>
             ) : (
               <div className="columns-1 md:columns-2 gap-6 md:gap-8 space-y-6 md:space-y-8">
                 {posts.map(p => <PostCard key={p.id} post={p} currentUser={currentUserData} onDelete={handleDeletePost} />)}
               </div>
             )}
